...
map<pii,int> edges;  // mapping edge to their weight
//vector<we> edges;   // the vector of edges
map<int,pii> edges_idx;   // index of the edges
int idx;    // later it will hold the total number of edges
...
int get_child_count(int a)  // for the i'th node
{
  if( child_count[a]!=-1 ) return child_count[a];
  if( g[a].size()==0 ){ child_count[a]=1; return 1; }
  int num=1;
  VI::iterator vit;
  for(vit=g[a].begin();vit!=g[a].end(); vit++)
    num+=get_child_count(*vit);
  child_count[a]=num;
  return num;
}
...
{
  VI va,vb; int i,j;
  i=a; while(i!=head_idx) va.pb(i), i=rg[i] ; va.pb(i);
  i=b; while(i!=head_idx) vb.pb(i), i=rg[i] ; vb.pb(i);
  i=va.size()-1;  j=vb.size()-1;
  while( va[i]==vb[j] && i>=0 ) i--,j--;  i++,j++; return va[i];
}

// l,r -> CAUTION: they are index
void init(int p,int tt,int l,int r)   // p -> the path head
{
  if( l==r ){ segT[p][tt]=paths_el[p][l];return  ; }
  int mid=(l+r)>>1;
  init(p,tt<<1,l,mid);
...
void update(int p,int tt,int l,int r,int x,int a)  // update the x'th element to a
{
  if( l==r && x==l ) {segT[p][tt]=a; return ; }
  int mid=(l+r)>>1;
  if( x<=mid ) update(p,tt<<1,l,mid,x,a),segT[p][tt]=max(segT[p][tt<<1],segT[p][(tt<<1)|1]);
  else if( x>mid ) update(p,(tt<<1)|1,mid+1,r,x,a),segT[p][tt]=max(segT[p][tt<<1],segT[p][(tt<<1)|1]) ;
}
  
inline void test()
{
  sci(n);
  int i,j;int t1,t2,t3,tmp1,tmp2,tmp3; idx=0; 
  fill(rg,-1); edges.clear(); fill(child_count,-1); paths.clear();path_head.clear(); path_count.clear(); fill(path_pos,-1);fill(curr_head,-1);fill(height,-1); edges_idx.clear(); g.clear();paths_el.clear();
  REP(i,1,n) g[i]=VI();
  FOR(i,1,n)
    {
      sc2i(t1,t2);  sci(t3);
  ...
  REP(i,1,n)  if(rg[i]==-1){ head_idx=i;break; }   // find the head of the tree
  rg[head_idx]=head_idx, edges[mp(head_idx,head_idx)]=0; 

  get_child_count(head_idx);

  // find the paths
  queue<int> q;  // the head of bfs
  q.push(head_idx); height[head_idx]=0;
  int mx,mxi;
  VI::iterator vit;
  while( !q.empty() )
    {
      ...
      else path_pos[tmp1]=path_count[curr_head[tmp1]]++ , paths[curr_head[tmp1]].pb(tmp1), paths_el[curr_head[tmp1]].pb(edges[mp(rg[tmp1],tmp1)]);
  ...
  FOR(i,0,path_head.size()) segT[path_head[i]]=VI(paths[path_head[i]].size()*6,0) , init(path_head[i],1,0,paths_el[path_head[i]].size()-1);
